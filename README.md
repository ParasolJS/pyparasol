...
-	columns_to_hide (list, optional) needs to be entered as a list of column headers relating to column headers in the data file and is optional. This optional parameter sets which columns from the inputted data file you don’t want to show.
-	axes_layout (list, optional) needs to be entered as a list of column headers relating to the column headers in the data file and is optional. This parameter specifies exactly which columns from the inputted data file you want to have showing in the plot. Something important to note is that **axes_layout will override columns_to_hide.**
-	axes_to_flip (list, optional) needs to be entered as a list of column headers relating to the column headers in the data file and is optional. This parameter specifies axes to be flipped in the Parasol application. The default is displaying data in increasing order from bottom to top.
-	plot_color (string, optional): the color of the data in the plot, entered as a 6 character hex value.
-	brushed_color (string, optional): the color of the data in the plot when brushed, entered as a 6 character hex value.
-	plot_alpha (float, optional): the alpha value of the plot, entered as a float between 0 and 1.
-	brushed_alpha (float, optional): the alpha value of the plot when brushed, entered as a float between 0 and 1.
-	reorderable_status (boolean, optional): determines the status of setting the reorderable attribute to the plot.
...
-	reorder_status: boolean, plot_id_list: list (optional).
...
-	plot_alpha: float in the range (0, 1], plot_id_list: list (optional).
...
-	alpha_on_brushed: float in the range (0, 1], plot_id_list: list (optional).
...
-	plot_color: 6 character hex color code (string), plot_id_list: list (optional).
...
-	brushed_color: 6 character hex color code (string), plot_id_list: list (optional).
...
-	new_html_file_name: string in the form of a html file name ("example.html").
...
-	linked_status: boolean, plot_id_list: list (optional).
...
-	cluster_status: boolean, variables_to_cluster: list (optional), number_colors: integer (optional), plot_id_list: list (optional).
...
-	variable_list: list, associated_weights: list, plot_id_list: list (optional).
...
-	variable_list: list, scale_list: list, plot_id_list: list (optional).
...
-	html_file_name: string (optional).
...
**PyParasol.addExportAllButton()**
-	Adds a button to the Parasol application that will export all data.
...
**PyParasol.addKeepSelectedButton()**
-	Adds a button to the Parasol application that will keep only the current data selections.

**PyParasol.addRemoveSelectedButton()**
-	Adds a button to the Parasol application that will remove the current data selections.
